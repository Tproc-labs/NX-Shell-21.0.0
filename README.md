Freetype-config is broken, doesn't exist or something:
FIX: 
```
# Create the freetype-config wrapper
cat > /opt/devkitpro/portlibs/switch/bin/freetype-config << 'EOF'
#!/bin/bash
case "$1" in
  --libs)
    aarch64-none-elf-pkg-config --libs freetype2
    ;;
  --cflags)
    aarch64-none-elf-pkg-config --cflags freetype2
    ;;
  *)
    echo "Usage: freetype-config [--libs|--cflags]"
    exit 1
    ;;
esac
EOF

chmod +x /opt/devkitpro/portlibs/switch/bin/freetype-config
